...
with open('../MOCK_DATA/MOCK_RESIDENT_DATA.csv', newline='') as residents_file:
    ...
    for row in reader:
        ...
        curs.execute("INSERT OR IGNORE INTO tb_resident_info (first_name,last_name,email,phone,birthdate,gender) VALUES (?,?,?,?,?,?)", row)
...
curs.executemany("INSERT OR IGNORE INTO tb_housing_complexes VALUES (?,?,?)", complexes)
...
with open('../MOCK_DATA/MOCK_UNITS_DATA.csv', newline='') as units_file:
    ...
    for row in reader:
        row[0] = int(row[0])
        row[6] = float(row[6])
        curs.execute("INSERT OR IGNORE INTO tb_student_housing_units VALUES (?,?,?,?,?,?,?)", row)
...
curs.executemany("INSERT OR IGNORE INTO tb_leases VALUES (?,?,?,?,?)", leases)
conn.commit()
